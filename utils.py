...
def get_neighs_degree_matrix(data: torch_geometric.data.data.Data, edge_type):
    ...
    # add degree_exist_matrix with data.x
    degree_exist_matrix = torch.as_tensor(degree_exist_matrix, dtype=torch.float32)
    linear = nn.Linear(data.degree_max, data.num_node_features)
    degree_exist = linear(degree_exist_matrix).data
    data.x = data.x + degree_exist
    return data


def set_data_mask(data: torch_geometric.data.data.Data, train_ratio=0.8, pred_ratio=0.01):
    """
    ...
    test_mask[int(train_ratio * data.y.size(0)):] = True
