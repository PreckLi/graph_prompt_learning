...
class MEWISPool(nn.Module):
    ...
    def graph_reconstruction(mewis, x, A):
        ...
        adj_pooled = (one - I).to(device) * torch.clamp(A2 + A3, min=0, max=1)
...
class Net3Prompt(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes):
        super(Net3Prompt, self).__init__()
    ...
    def forward(self, x, edge_index, batch):
        ...
        out = self.fc1(readout)
        out = torch.relu(out)
        result1 = self.fc2(out)

        result2 = self.lin4(out).squeeze(-1)
        result3 = self.lin5(out).squeeze(-1)

        return [torch.log_softmax(result1, dim=-1),result2,result3], loss1 + loss2

class Net2Prompt(nn.Module):
    def __init__(self, input_dim, hidden_dim,distribution_shape, num_classes):
        super(Net2Prompt, self).__init__()
        ...
        self.fc2 = nn.Linear(in_features=hidden_dim, out_features=num_classes)
        # node neighborhoods' degree prediction linear
        self.lin4 = nn.Linear(hidden_dim, distribution_shape)
        self.loss_linear = nn.Linear(hidden_dim, 1)

    def forward(self, x, edge_index,degree, batch):
        x = self.gc1(x, edge_index)
        x = torch.relu(x)
        ...
        result3 = self.lin4(out).squeeze(-1)

        return [torch.log_softmax(result1, dim=-1),mse_loss,result3],loss1