...
def read_tu_data(folder, prefix):
    ...
    data = get_graph_degree_diffpool(data, slices)  # for diffpool
...
def get_graph_feature(data: torch_geometric.data.Data, slices: dict):
    ...
    data.graph_degree_distribution = torch.as_tensor(degree_exist_matrix, dtype=torch.float32)
    return data


def get_graph_degree_diffpool(data: torch_geometric.data.Data, slices: dict):
    edge_slice = slices['edge_index'].numpy()
    edge_index = data.edge_index
    node_degree_list = list()
    for i in range(len(edge_slice) - 1):
        ...
        node_degree_list.append(degree_log)
    max_length = 0
    for i in node_degree_list:
        if max_length < len(i):
            max_length = len(i)
    new_node_degree_list = list()
    for i in node_degree_list:
        pad = torch.as_tensor([0 for _ in range(abs(max_length - len(i)))], dtype=torch.float32)
        new_node_degree_list.append(torch.cat((i, pad)))
    data.degree = new_node_degree_list
    return data
