...
class DiffPool_Prompt(torch.nn.Module):
    def __init__(self, dataset, num_layers, hidden, ratio=0.25):
        super().__init__()
        num_nodes = ceil(ratio * dataset[0].num_nodes)
        self.embed_block1 = Block(dataset.num_features, hidden, hidden)
        ...
        self.lin3 = Linear(hidden, 1)
        self.lin4 = Linear(hidden, dataset.data.graph_degree_distribution.shape[1])
    ...
    def forward(self, data):
        x, adj, mask, degree = data.x, data.adj, data.mask, data.degree

        s = self.pool_block1(x, adj, mask)
        x = F.relu(self.embed_block1(x, adj, mask))
        xs = [x.mean(dim=1)]
        mse_x = self.lin3(x)
        mse_loss = F.mse_loss(mse_x.view(mse_x.shape[0], -1), degree)
